...
class InitiateOAuthCommand:
    ...
    async def execute(self, provider: str, user_id: uuid.UUID, redirect_uri: str) -> OAuthURLDTO:
        authorize_url, state, code_verifier = await self.oauth_provider_adapter.get_authorize_url(redirect_uri)
        await self.state_repo.save(state, {"provider": provider, "user_id": str(user_id), "code_verifier": code_verifier})
...
class HandleCallbackCommand:
    ...
    async def execute(self, code: str, state: str) -> None:
        ...
        connection = OAuthConnection(
            ...
            user_id=user_id,
            provider=provider,
            access_token=EncryptedToken(value=self.encryption_service.encrypt(tokens["access_token"])),
            refresh_token=EncryptedToken(value=self.encryption_service.encrypt(tokens["refresh_token"])) if tokens.get("refresh_token") else None,
